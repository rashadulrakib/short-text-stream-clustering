...
def cluster_biterm(f, list_pred_true_words_index_postid_createtime, c_bitermsFreqs={}, c_totalBiterms={}, c_wordsFreqs={}, c_totalWords={}, c_txtIds={}, c_clusterVecs={}, txtId_txt={}, last_txtId=0, max_c_id=0, wordVectorsDic={}, dic_clus__id={}, dic_biterm__clusterId_Freq={}, dic_biterm__allClusterFreq={}, dic_biterm__clusterIds={}, c_textItems={}, dic_ngram__textItems={}, min_gram=1, max_gram=2):
  ...
  for item in list_pred_true_words_index_postid_createtime:
    ...
    max_c_id=max([max_c_id, clusterId,len(c_bitermsFreqs)])+1
    ...
    if line_count%500==0:
      ...
      for c_id, orderId in dic_clus__id.items():
        ...
        if ( float(c_id)<=float(abs(mean_c_id-std_c_id)) or float(orderId)<=float(abs(mean_c_id-std_c_id))) or (c_size<=1 or float(c_size)<=float(abs(mean_c_size-std_c_size))) or (float(c_size)>=mean_c_size+std_c_size*1):		
          list_del_cids.append(c_id)  		
  ...
  return [c_bitermsFreqs, c_totalBiterms, c_wordsFreqs, c_totalWords, c_txtIds, c_clusterVecs, txtId_txt, last_txtId, dic_clus__id, dic_biterm__clusterId_Freq, dic_biterm__allClusterFreq, dic_biterm__clusterIds, c_textItems, dic_ngram__textItems]  
