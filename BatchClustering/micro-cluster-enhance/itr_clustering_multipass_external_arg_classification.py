...
maxPercent = 90
percentIncr = 10
cluster_result_file_id = "4" #str(sys.argv[1])
print(cluster_result_file_id)
minP = 50 #float(str(sys.argv[2]))*100
print(minP)
fixedMaxP = 95 #float(str(sys.argv[3]))*100
print(fixedMaxP)
maxTrainRatio =fixedMaxP/100
job_folder= "batchId_PredTrueText10_to_16" #"NewsPredTueTextMStreamSemantic_WordArr" #+cluster_result_file_id #str(sys.argv[4])
...
#dataFileTxtTrue = "/dshome/grad/rakib/dataset/shorttext/tweet/2472/tweet-original-order.txt" #input
#extClustFile = "/dshome/grad/rakib/PhD/clustering-k-means/tweet-we-kmeans-run"+cluster_result_file_id #input

dataFilePredTrueTxt="/home/owner/PhD/MStream-master/MStream/result/batchId_PredTrueText10_to_16.txt" #NewsPredTueTextMStreamSemantic_WordArr.txt" #"batchId_PredTrueText"+cluster_result_file_id
...
def ComputePurity(dic_tupple_class):
 ...
 purity=float(maxGroupSizeSum)/float(totalItems)
...
listtuple_pred_true_text = ReadPredTrueText(dataFilePredTrueTxt)
numberOfClusters=ObtainNumberOfClusters(False, listtuple_pred_true_text) #by predicted clusters
WriteTrainTestInstances(traintestFile, listtuple_pred_true_text) #write one time
print("numberOfClusters by predicted clusters="+str(numberOfClusters))
