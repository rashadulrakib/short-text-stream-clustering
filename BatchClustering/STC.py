...
class ClusteringLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        input_dim = input_shape[1]#input_shape[1].value
...
class STC(object):
    ...
    def fit(self, x, y=None, maxiter=2e4, batch_size=256, tol=1e-3,
        ...
        print("len=x,"+str(len(x))+", shape="+str(x.shape))
        ...
        print("before itr= acc", acc, "nmi", nmi)		
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ae_weights', default='/data/ae_weights.h5')
    ...
    # load dataset
    ####################################################################################
    fi=open("data/Stackoverflow-mstream", "r")
    lines=fi.readlines()
    fi.close()
    lines_word_arr=[]
    trueLabels=[]	
    for line in lines:
      ...
      lines_word_arr.append(word_tokenize(line['textCleaned']))
      trueLabels.append(int(line['clusterNo']))	  
    	
    x = load_data(wordVectorsDic, lines_word_arr)	
    y=np.array(trueLabels)	
    #x, y = load_data(args.dataset, wordVectorsDic)
    n_clusters = 20 # len(np.unique(y))
