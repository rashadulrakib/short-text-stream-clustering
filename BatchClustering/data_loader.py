...
def load_stackoverflow(wordVectorsDic, word_count):
...
def load_data(wordVectorsDic, lines_word_arr):
    word_count = Counter()
    for word_sentence in lines_word_arr:
    ...
    all_vector_representation = np.zeros(shape=(len(lines_word_arr), 300))
    i=-1	
    for word_sentence in lines_word_arr:
        i=i+1	
        ...
        for word in word_sentence:
            try:
                wv = np.array(wordVectorsDic[word])
                #print(wv)				
                j = j + 1
            except KeyError:
                continue
    ...
    #rakib: all_vector_representation=glove and 300 dim
