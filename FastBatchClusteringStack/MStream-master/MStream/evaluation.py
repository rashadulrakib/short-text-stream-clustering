...
def Evaluate(listtuple_pred_true_text, ignoreMinusOne=False):
    ...
    print("pred clusters=" + str(
        len(groupItemsBySingleKeyIndex(new_listtuple_pred_true_text, 0))) + ", true clusters=" + str(
        len(dic_tupple_class_true)))
...
def Evaluate_old(listtuple_pred_true_text, ignoreMinusOne=False):
    ...
    print('wrong:avg words per text', totalwords / len(new_listtuple_pred_true_text), 'totalwords', totalwords, '#texts',
          len(new_listtuple_pred_true_text))

    keysByLength = sorted(dic_tupple_class, key=lambda key: len(dic_tupple_class[key]), reverse=True)
    li = []
